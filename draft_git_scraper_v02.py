...
def get_pr_dates(repo_name: str, pr_number: int) -> Tuple[str, str]:
    ...
    if response.status_code == 403 and response.headers.get("X-RateLimit-Remaining") == "0":
        ...
        logging.error(f"Rate limit exceeded. Try again after {reset_time}.")
        raise GitHubRateLimitExceeded(f"Rate limit exceeded. Retry after {reset_time}")
...
def explode_base_commits(df: pd.DataFrame) -> pd.DataFrame:
    # поля, которые могут быть сериализованы как строки и являются списками списков
...
def main() -> None:
    ...
    csv_filename = 'sympy_sympy_commits_0011.csv'
